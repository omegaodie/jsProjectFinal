...
Level.prototype.AiDisperse = function(){
	for(i = 0; i < this.enemies.length; i++){
		var xd = this.enemies[i].sendPositionX();
	   var yd = this.enemies[i].sendPositionY();
		for(g = 0; g < this.enemies.length; g++){
		if(this.enemyDone[g] == false){
			if(i != g){
				var eX = this.enemies[g].sendPositionX();
	   	   var eY = this.enemies[g].sendPositionY();
			   if((xd > eX - 32) && (xd <  eX + 32)){
					if((yd > eY - 32) && (yd < eY + 32)){
						this.enemies[i].budge(eX + 3.14159, eY + 3.14915);
						this.enemies[g].budge(xd - 3.14915,yd - 3.14915);
				}
			}
			}
		}
...
Level.prototype.AIUpdate = function(){
	...
	for(i = 0; i < this.enemies.length; i++){
		if(this.enemyDone[i] == false){
			...
			if((this.player.sendPositionX() > this.enemies[i].sendPositionX() - 96) && (this.player.sendPositionX() <  this.enemies[i].sendPositionX() + 96)){
				if((this.player.sendPositionY() > this.enemies[i].sendPositionY() - 96) && (this.player.sendPositionY() < this.enemies[i].sendPositionY() + 96)){
					zombieLunge = true;
				}
			}
	...
	for (var i = 0; i < this.boxes.length; i++){
		if(this.boxDone[i] == false){
			...
			if((pX + 16 >= bX - 64 && pX - 16 <= bX + 64 && pY + 16 >= bY -64 && pY -16 <= bY + 64)
			   ||(pX - 16 <= bX + 64 && pX + 16 >= bX - 64 && pY - 16 <= bY - 64 && pY + 16 >= bY - 64)){
		...
		for(var i = 0; i < this.boxes.length; i++){
			{
				...
				if(this.boxes[i].sendMove() == 'left'){
					if(this.boxes[i].sendPositionX()  < 90){
						this.boxes[i].Stop();
						this.boxes[i].setMove(30, this.boxes[i].sendPositionY());
					}
				}
	...
	for(g = 0; g < 4; g++){
		...
		for(q = 0; q < 4; q++){
			if(this.boxDone[q] == false){
				...
				if((pX + 16 >= bX && pX <= bX + 64 && pY + 16 >= bY && pY <= bY + 64)||(pX - 16 <= bX && pX >= bX - 64 && pY - 16 <= bY && pY >= bY - 64)){
					if(this.CL == 0){
						this.boxes[q].Stop();
					}
					if(this.CL == 1){
